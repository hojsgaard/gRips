library(unifips)
#source("helpers.R")
library(gRbase)
library(microbenchmark)
#load_all("unifips")

##build("unifips");install("unifips")
## math

data(math)
dat <- scale(math)
LAMBDA  <- .3
glasso_ns(dat, lambda=LAMBDA)
glasso_ns(dat, lambda=LAMBDA)
glasso_ns(dat, lambda=LAMBDA)
g <- glasso_ns(dat, lambda=LAMBDA)
plot(g)


## prostate
USE   <- 1:10
LAMBDA  <- .3

indat <- read.csv("data/prostate.csv")
dat   <- as.matrix(scale(indat[,USE]))
nc    <- ncol(dat)
vn    <- colnames(dat)
nobs <- nrow(dat)
#S <- .make_prostateS()[USE, USE]
S <- cov(dat)
attr(S, "n") <- nrow(dat)
## Saturated model
esat <- t(names2pairs(1:nrow(S), result="matrix"))

load_all("unifips")
system.time({e1b = unifips18(esat, S, iter=2, eps=0.01)})

e3 <- glasso_ns(dat, lambda=LAMBDA)

r3 <- unifips18(e3$emat, S)
r4 <- unifips18(esat, S, aux=list(method="lasso", lambda=LAMBDA))
r5 <- unifips18(esat, S, aux=list(method="lasso", lambda=LAMBDA, penalize.diagonal=F))

## Saturated model
esat

## Should be the same (and they are)
e3$e
r3$e

## Should be similar to above and they are
r4$e
r5$e

load_all("unifips")
gl1 <- glasso::glasso(S, rho=LAMBDA, penalize.diagonal=!TRUE) %>% iznogood()
un1 <- unifips18(esat, S, aux=list(method="lasso", lambda=LAMBDA, penalize.diagonal=!TRUE))
cg1 <- cheap_glasso(dat, lambda=LAMBDA)

K.gl1 <- gl1$K
K.un1 <- un1$K

gl1$emat
un1$emat


LAMBDA=.3
load_all("unifips")
microbenchmark::microbenchmark(
e1a = glasso::glasso(S, rho=LAMBDA, penalize.diagonal=!FALSE),
e1b = unifips18(esat, S, aux=list(method="lasso", lambda=LAMBDA)),
e1d = unifips18(esat, S, check=2, aux=list(method="lasso", lambda=LAMBDA)),
e1c = cheap_glasso(dat, lambda=LAMBDA),
ed = unifips18(esat, S, aux=list(method="mtp2")),
times=5)


e1b = unifips18(esat, S, aux=list(method="lasso", lambda=LAMBDA))
e1d = unifips18(esat, S, check=2, aux=list(method="lasso", lambda=LAMBDA))

e1b
e1d


## Caroline data

load("data/lcm.RData")
load("data/ldm.RData")

USE <- 1:500
dat <- ldm[, USE]
S <- cov2cor(lcm)[USE, USE]
esat <- t(names2pairs(1:nrow(S), result="matrix"))
load_all("unifips")

LAMBDA <- .3

system.time({
    gl1 <- glasso::glasso(S, rho=LAMBDA, penalize.diagonal=TRUE) %>% iznogood()})

system.time({
    un1 <- unifips18(esat, S, aux=list(method="lasso", lambda=LAMBDA, penalize.diagonal=TRUE))})


system.time({
    un2 <- unifips18(esat, S, aux=list(method="mtp2"))
})


cg1 <- cheap_glasso(dat, lambda=LAMBDA)

lpen <- function(K, S, LAMBDA){
   log(det(K)) - sum(K*S) - sum(LAMBDA * abs(K))
}


L <- matrix(LAMBDA, nrow=nrow(S), ncol=ncol(S)); diag(L) <- 0

lpen(gl1$K, S, L)
lpen(un1$K, S, L)

d <- gl1$K - un1$K

which(abs(d) > 1e-4, arr.ind=T)


e1  <- gl1$emat
e2  <- un1$emat


n1 <- apply(e1 * c(1, 10000),2, sum)

n2 <- apply(e2 * c(1, 10000),2, sum)

intersect(n1, n2)
